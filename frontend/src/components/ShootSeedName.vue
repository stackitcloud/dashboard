      ...
      <template v-slot:activator="{ on }">
        <div v-on="on">
          <template v-if="isControlPlaneMigrating">
            ...
            <router-link v-if="canLinkToSeed(shootStatusSeedName)" class="primary--text" :to="{ name: 'ShootItem', params: { name: shootStatusSeedName, namespace:'garden' } }">
              <span>{{shootStatusSeedName}}</span>
            </router-link>
            <span v-else>{{shootSeedName}}</span>
            <v-icon small class="mx-1">mdi-arrow-right</v-icon>
          </template>
          <router-link v-if="canLinkToSeed(shootSeedName)" class="primary--text" :to="{ name: 'ShootItem', params: { name: shootSeedName, namespace:'garden' } }">
